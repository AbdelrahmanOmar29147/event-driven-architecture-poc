...
public class GatewayProducer {
    ...
    public void sendMessage(RefundInput refundInput) {
        kafkaTemplate.send(Topics.PAYMENT_REFUND_REQUEST, new EventModel<RefundInput>(refundInput.getRequestId(), refundInput.getMerchantRefNum(), refundInput));
    }
    public void sendMessage(WebsocketMessage message) {
        kafkaTemplate.send(Topics.PAYMENT_ORDER_REQUEST, new EventModel<WebsocketMessage>(message.getMessageId(), message.getMerchantRefNum(), message));
    }
}
